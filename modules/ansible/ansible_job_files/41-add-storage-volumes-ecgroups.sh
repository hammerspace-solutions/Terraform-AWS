...
flag="0"  # Initialize flag for ecgroup_servers parsing
while read -r line; do
...
echo "--- Existing Volumes: $existing_volumes_json"
all_ecgroup_servers_json=$(printf '%s\n' "${all_ecgroup_servers[@]}" | jq -R . | jq -s .)
echo "--- All ECGroup Servers JSON: $all_ecgroup_servers_json"
...
- hosts: localhost
  ...
  vars:
    ...
    ecgroup_servers: $all_ecgroup_servers_json
              ...
              {% if full_name not in existing_volumes %}
                {
                  ...
                  }{% if ecgroup_servers | length > 0 %},
                  "additionalAddresses": [
                    {% for ip in ecgroup_servers %}
                    {
                      "ip": {
                        "address": "{{ ip }}",
                        "prefixLength": 32
...
EOF
  ...
  echo "Running Ansible playbook to add ECGroup storage volumes..."
  playbook_output=$(ansible-playbook "$tmp_playbook" 2>&1)
  echo "--- Playbook Output: $playbook_output"
  ...
  echo "--- Added Volumes JSON: $added_volumes_json"
  ...
  if [ "$added_volumes_json" != "[]" ] && [ -n "$added_volumes_json" ]; then
    ...
    echo "--- Added Volumes JSON after editing: $added_volumes_json"
    added_volumes=$(echo $added_volumes_json | jq -r '.[]' 2>/dev/null)
    echo "--- Added Volumes: $added_volumes"
  ...
  echo "--- Add ECGroup Storage Volumes Job Complete ---"
